...
export async function set(key, value, opts = {}) {
  ...
  // Upstash REST options are bare flags / key=value, NOT URLSearchParams:
  // `?EX=300` works but `?NX=true` is a syntax error — NX must be a bare flag.
  const parts = [];
  if (opts.ex) parts.push(`EX=${Number(opts.ex)}`);
  if (opts.nx) parts.push("NX");
  const q = parts.length ? "?" + parts.join("&") : "";
  const r = await fetch(`${URL}/set/${encodeURIComponent(key)}${q}`, {
    method: "POST",
    headers: H,
    body,
  });
